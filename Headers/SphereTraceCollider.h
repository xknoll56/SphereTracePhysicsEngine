...
{
	ST_Vector3 dp1 = sphereTraceVector3Subtract(pSphereCollider->pRigidBody->position, aabb->rightTopForwardsTransformedVertex);
	ST_Vector3 dp2 = sphereTraceVector3Subtract(pSphereCollider->pRigidBody->position, aabb->leftDownBackTransformedVertex);
	ST_Vector3 closestPoint;
	if (fabsf(dp1.x) <= fabsf(dp2.x))
	{
		if (fabsf(dp1.y) <= fabsf(dp2.y))
		{
			if (fabsf(dp1.z) <= fabsf(dp2.z))
			{
				closestPoint = aabb->rightTopForwardsTransformedVertex;
			}
			else
			{
				closestPoint = (ST_Vector3){ aabb->rightTopForwardsTransformedVertex.x, aabb->rightTopForwardsTransformedVertex.y, aabb->leftDownBackTransformedVertex.z };
			}
		}
		else
		{
			if (fabsf(dp1.z) <= fabsf(dp2.z))
			{
				closestPoint = (ST_Vector3){ aabb->rightTopForwardsTransformedVertex.x, aabb->leftDownBackTransformedVertex.y, aabb->rightTopForwardsTransformedVertex.z };
			}
			else
			{
				closestPoint = (ST_Vector3){ aabb->rightTopForwardsTransformedVertex.x, aabb->leftDownBackTransformedVertex.y, aabb->leftDownBackTransformedVertex.z };
			}
		}
	}
	else
	{
		if (fabsf(dp1.y) <= fabsf(dp2.y))
		{
			if (fabsf(dp1.z) <= fabsf(dp2.z))
			{
				closestPoint = (ST_Vector3){ aabb->leftDownBackTransformedVertex.x, aabb->rightTopForwardsTransformedVertex.y, aabb->rightTopForwardsTransformedVertex.z };
			}
			else
			{
				closestPoint = (ST_Vector3){ aabb->leftDownBackTransformedVertex.x, aabb->rightTopForwardsTransformedVertex.y, aabb->leftDownBackTransformedVertex.z };
			}
		}
		else
		{
			if (fabsf(dp1.z) <= fabsf(dp2.z))
			{
				closestPoint = (ST_Vector3){ aabb->leftDownBackTransformedVertex.x, aabb->leftDownBackTransformedVertex.y, aabb->rightTopForwardsTransformedVertex.z };
			}
			else
			{
				closestPoint = aabb->leftDownBackTransformedVertex;
			}
		}
	}

	if (sphereTraceVector3Length2(sphereTraceVector3Subtract(closestPoint, pSphereCollider->pRigidBody->position)) <= pSphereCollider->radius * pSphereCollider->radius)
		return 1;
	return 0;
...
void sphereTraceColliderTriangleSetVertexAndEdgeData(ST_TriangleCollider* const pTriangleCollider, ST_Vector3 v1, ST_Vector3 v2, ST_Vector3 v3)
{
	...
	pTriangleCollider->edgeCenterDirs[0] = sphereTraceVector3Normalize(sphereTraceVector3Subtract(sphereTraceVector3Average(pTriangleCollider->transformedEdges[0].point1, pTriangleCollider->transformedEdges[0].point2), pTriangleCollider->centroid));
	pTriangleCollider->edgeCenterDirs[1] = sphereTraceVector3Normalize(sphereTraceVector3Subtract(sphereTraceVector3Average(pTriangleCollider->transformedEdges[1].point1, pTriangleCollider->transformedEdges[1].point2), pTriangleCollider->centroid));
	pTriangleCollider->edgeCenterDirs[2] = sphereTraceVector3Normalize(sphereTraceVector3Subtract(sphereTraceVector3Average(pTriangleCollider->transformedEdges[2].point1, pTriangleCollider->transformedEdges[2].point2), pTriangleCollider->centroid));
...
b32 sphereTraceColliderTriangleRayTrace(ST_Vector3 from, ST_Vector3 dir, const ST_TriangleCollider* const pTriangleCollider, ST_RayTraceData* const pRaycastData)
{
	...
	if (pRaycastData->distance >= 0.0f)
	{
		...
		pRaycastData->hitPoint = sphereTraceVector3Add(from, sphereTraceVector3Scale(dir, pRaycastData->distance));
		//float dot1 = sphereTraceVector3Dot(pTriangleCollider->normal, sphereTraceVector3Cross(sphereTraceVector3Subtract(pTriangleCollider->transformedVertices[0], pTriangleCollider->transformedVertices[2]),
		//	sphereTraceVector3Subtract(pRaycastData->hitPoint, pTriangleCollider->transformedVertices[2])));
		//float dot2 = sphereTraceVector3Dot(pTriangleCollider->normal, sphereTraceVector3Cross(sphereTraceVector3Subtract(pTriangleCollider->transformedVertices[1], pTriangleCollider->transformedVertices[0]),
		//	sphereTraceVector3Subtract(pRaycastData->hitPoint, pTriangleCollider->transformedVertices[0])));
		//float dot3 = sphereTraceVector3Dot(pTriangleCollider->normal, sphereTraceVector3Cross(sphereTraceVector3Subtract(pTriangleCollider->transformedVertices[2], pTriangleCollider->transformedVertices[1]),
		//	sphereTraceVector3Subtract(pRaycastData->hitPoint, pTriangleCollider->transformedVertices[1])));
		//dot1 = copysignf(1.0f, dot1);
		//dot2 = copysignf(1.0f, dot2);
		//dot3 = copysignf(1.0f, dot3);
		//if (dot1==dot2==dot3)
...
b32 sphereTraceColliderPlaneSphereTrace(ST_Vector3 from, ST_Vector3 dir, float radius, const ST_PlaneCollider* const pPlaneCollider, ST_SphereTraceData* const pSphereCastData)
{
	...
	else
	{
		...
		else
		{
			...
			theta = acosf(sphereTraceVector3Dot(sphereTraceVector3Negative(dir), orthogonal));
			hypotinus = radius / sinf(theta);
		}
		...
		ST_Vector3 dp = sphereTraceVector3Subtract(testPoint, pPlaneCollider->position);
		...
		{
			ST_PlaneEdgeDirection closestEdge = sphereTraceColliderPlaneGetClosestTransformedEdgeToPoint(pPlaneCollider, pSphereCastData->rayTraceData.hitPoint);
			...
			if (distToCenterNormal <= radius && edgeDist>=0.0f && edgeDist<=maxDist)
			{
				pSphereCastData->rayTraceData.startPoint = from;
			...
			{
				//check point
				ST_Vector3 closestPoint = pPlaneCollider->transformedVertices[sphereTraceColliderPlaneGetClosestTransformedVertexToPoint(pPlaneCollider, pSphereCastData->rayTraceData.hitPoint)];
				dp = sphereTraceVector3Subtract(pSphereCastData->rayTraceData.hitPoint, closestPoint);
				cross = sphereTraceVector3Normalize(sphereTraceVector3Cross(sphereTraceVector3Normalize(dp), dir));
				cross = sphereTraceVector3Cross(dir, cross);
				distToCenterNormal = sphereTraceVector3Dot(dp, cross);
				if (distToCenterNormal <= radius)
				{
					pSphereCastData->rayTraceData.startPoint = from;
...
b32 sphereTraceColliderTriangleSphereTrace(ST_Vector3 from, ST_Vector3 dir, float radius, const ST_TriangleCollider* const pTriangleCollider, ST_SphereTraceData* const pSphereCastData)
{
	...
	else
	{
		...
		if (sphereTraceColliderTriangleIsProjectedPointContained(testPoint, pTriangleCollider))
		{
			pSphereCastData->rayTraceData.hitPoint = testPoint;
			pSphereCastData->rayTraceData.startPoint = from;
			pSphereCastData->rayTraceData.distance = sphereTraceVector3Length(sphereTraceVector3Subtract(pSphereCastData->rayTraceData.hitPoint, from));
		...
		{
			int closestEdgeIndex = sphereTraceColliderTriangleGetClosestTransformedEdgeIndexToPoint(pTriangleCollider, pSphereCastData->rayTraceData.hitPoint);
			ST_Vector3 edgeDir = pTriangleCollider->edgeDirs[closestEdgeIndex];
			ST_Vector3 closestPointOnEdge = sphereTraceClosestPointOnLineBetweenTwoLines(pTriangleCollider->transformedEdges[closestEdgeIndex].point1,
				edgeDir,
				pSphereCastData->rayTraceData.hitPoint, dir);
			float maxDist = pTriangleCollider->edgeDists[closestEdgeIndex];
			ST_Vector3 dp = sphereTraceVector3Subtract(pSphereCastData->rayTraceData.hitPoint, closestPointOnEdge);
			ST_Vector3 cross = sphereTraceVector3Normalize(sphereTraceVector3Cross(edgeDir, dir));
			float distToCenterNormal = sphereTraceVector3Dot(dp, cross);
			float edgeDist = sphereTraceVector3Dot(edgeDir, sphereTraceVector3Subtract(closestPointOnEdge, pTriangleCollider->transformedEdges[closestEdgeIndex].point1));
			if (distToCenterNormal <= radius && edgeDist >= 0.0f && edgeDist <= maxDist)
			{
				pSphereCastData->rayTraceData.startPoint = from;
				pSphereCastData->rayTraceData.hitPoint = closestPointOnEdge;
			...
			{
				//check point
				ST_Vector3 closestPoint = pTriangleCollider->transformedVertices[sphereTraceColliderTriangleGetClosestTransformedVertexIndexToPoint(pTriangleCollider, pSphereCastData->rayTraceData.hitPoint)];
				dp = sphereTraceVector3Subtract(pSphereCastData->rayTraceData.hitPoint, closestPoint);
				cross = sphereTraceVector3Normalize(sphereTraceVector3Cross(sphereTraceVector3Normalize(dp), dir));
				cross = sphereTraceVector3Cross(dir, cross);
				distToCenterNormal = sphereTraceVector3Dot(dp, cross);
...
ST_IntList sphereTraceColliderUniformTerrainSampleFirstTriangleIndicesWithAABB(const ST_UniformTerrainCollider* const terrainCollider, const ST_AABB* const paabb)
{
	...
	for (float z = paabb->leftDownBackTransformedVertex.z; z < maxCheckPos.y; z += terrainCollider->cellSize)
	{
		for (float x = paabb->leftDownBackTransformedVertex.x; x < maxCheckPos.x; x += terrainCollider->cellSize)
		{
			int index = sphereTraceColliderUniformTerrainSampleFirstTriangleIndex(terrainCollider, (ST_Vector3) { x, terrainCollider->aabb.leftDownBackTransformedVertex.y, z });
			if(index!=-1)
...
b32 sphereTraceColliderUniformTerrainSphereFindMaxPenetratingTriangle(const ST_UniformTerrainCollider* const pTerrainCollider, const ST_SphereCollider* const pSphereCollider, ST_SphereTriangleContactInfo* const pContactInfo)
{
	ST_IntListData* pIl;
	ST_IntList il = sphereTraceColliderUniformTerrainSampleFirstTriangleIndicesWithAABB(pTerrainCollider, &pSphereCollider->aabb);
	pIl = il.pFirst;
	...
	for (int i = 0; i < il.count; i++)
	{
		...
		{
			if (sphereTraceColliderAABBIntersectAABBVertically(&pSphereCollider->aabb, &pTerrainCollider->triangles[index].aabb))
			{
				if (sphereTraceColliderAABBIntersectSphere(&pTerrainCollider->triangles[index].aabb, pSphereCollider))
				{
					ST_SphereTriangleContactInfo ciTest;
					if (sphereTraceColliderTriangleSphereCollisionTest(&pTerrainCollider->triangles[index], pSphereCollider, &ciTest))
					{
		...
		index++;
		//sceneDrawTriangleOutline(&scene->terrainCollider.triangles[index], gVector4ColorGreen);
		if (!pTerrainCollider->triangles[index].ignoreCollisions)
		{
			if (sphereTraceColliderAABBIntersectAABBVertically(&pSphereCollider->aabb, &pTerrainCollider->triangles[index].aabb))
			{
